...
export PKG_DB_URL_PREFIX=${PKG_DB_URL_PREFIX:-"database"}
export PKG_DB=${PKG_DB:-"yaps.package"}
export PKG_URL_PREFIX=${PKG_URL_PREFIX:-"packages-10.0/$(uname -m)"}
...
yinfo() {
    ...
    local _appdb=$(cat $APPDB_PATH/"${PKG_DB}" | grep "^$appid")
    ...
    [[ -z $WITHOUTSYS ]] && {
        if isInstalled $appid; then
            ...
            if [[ ! -z "$_appversion" && $_appversion != $_sysver ]]; then
                _appversion="$_sysver -> $_appversion"
            else
                _appversion=$_sysver
            fi

            if [[ ! -z "$_apprelease" && $_apprelease != $_sysrel ]]; then
...
yupdate_mirror() {
    ...
    wget -O ${APPDB_PATH}/"${_mirror_id}" "${_mirror_url}/${PKG_DB_URL_PREFIX}/${_mirror_id}"
}
...
yupgrade() {
    __UPDATE_LIST__=""
    for i in $ROOTDIR/usr/share/$YDATADIR/*; do
...
ydownloader() {
    ...
    while read -r line ; do
        ...
        echo "downloading $_murl/$_path"
        wget $WGETFLAGS -q --show-progress "${_murl}/${PKG_URL_PREFIX}/$_path" -O "${PKGDIR}/$_path.part"
        if [[ $? != 0 ]] ; then
...
yinstaller() {
    ...
    local ypkg="${appid}-${_aver}-${_arel}-$(uname -m).$EXTENSION"
...
#   YDATADIR
ylist() {
    local _datadir="${APPDB_PATH}"

    for i in ${_datadir}/* ; do
        [[ -f "${i}" ]] || continue # skip if not a file
        while read -r line ; do
            local _appid="$(echo $line | cut -d ' ' -f1)"
            local _appdesc="$(echo $line | cut -d ' ' -f4 | tr '-' ' ')"
